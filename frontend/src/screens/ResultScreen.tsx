 ...
 * Экран результата.
 *
 * Показывается только после того, как шар лопнул, — даже если выигрыш был
 * зафиксирован раньше. Так игрок видит полную картину раунда: где он вышел и
 * где бы его ждал крах.
 *
 * Итог раунда состоит из трёх независимых частей, и все три показаны и при
 * выигрыше, и при проигрыше: бонусные баллы (деньги раунда), игровые очки
 * (турнирный прогресс) и фрагмент коллекции (долгосрочная награда). Проигрыш
 * обнуляет только первую из них — поэтому проигранный раунд всё равно
 * продвигает игрока.
 *
 * Через resultIdleTimeoutSeconds бездействия экран сам возвращает к выбору
 * темы, чтобы прототип не «залипал» на результате между проверяющими.
 */
export function ResultScreen(): JSX.Element {
  const { result, setup, player, playAgain, repeatBet, goTo, applyPurchase } = useGame();
  ...
  useEffect(() => {
    ...
    if (result.upsell.available) {
      // Небольшая пауза, чтобы предложение не перекрыло сам результат.
  ...
  // Таймер бездействия: любое действие пользователя сбрасывает отсчёт.
  useEffect(() => {
    if (upsellOpen) {
      return undefined;
    }
    ...
    <div className={`result result--${result.won ? 'win' : 'loss'}`}>
      <section className="panel panel--strong panel--pad result__main">
        <header className="result__head">
          ...
          <div className="col">
            ...
            <p className="text-sm muted">
              {result.won
                ? `Вы зафиксировали ${formatMultiplier(result.cashoutMultiplier)} при ставке ${formatNumber(result.betAmount)}.`
                : `Ставка ${formatNumber(result.betAmount)} не вернулась: «Забрать» не было нажато до краха.`}
            </p>
          </div>
        </header>
        ...
        <div className="result__numbers">
          <Metric
            label="Коэффициент краха"
            value={formatMultiplier(result.crashMultiplier)}
            tone="negative"
            hint="Точка, определённая сервером до старта"
          />
          <Metric
            label={result.won ? 'Ваш коэффициент' : 'Пройдено уровней'}
            value={result.won ? formatMultiplier(result.cashoutMultiplier) : `${result.levelsPassed} / ${result.levelCount}`}
            tone={result.won ? 'positive' : 'neutral'}
          />
          <Metric
            label="Максимум раунда"
            value={formatMultiplier(missed)}
            hint="Столько можно было забрать в самый последний момент"
          />
          <Metric
            label="Бустер"
            value={
              result.boostTier > 1
                ? result.boostApplied
                  ? `×${result.boostValue.toFixed(0)} на ${result.boostLevel} уровне`
        ...
        {result.won && (
          <p className="result__missed text-sm">
            Могли бы забрать больше: максимум раунда — {formatMultiplier(missed)}, это{' '}
        ...
        <div className="result__actions">
          ...
          <button
            ...
            title={
              player.bonusBalance < result.betAmount
                ? 'Не хватает бонусов на такую же ставку'
                : 'Тот же фрагмент, та же тема'
            }
            onClick={() => {
              audio.click();
              void repeatBet();
          ...
          >
            Повторить ставку {formatNumber(result.betAmount)}
          </button>
        </div>

        <p className="text-xs muted result__idle">
          Через {Math.max(0, idleLimit - idleSeconds)} с без действий вернёмся к выбору темы.
        </p>
      </section>

      <aside className="result__aside">
        <div className="panel panel--pad result__points">
          <span className="eyebrow">Игровые очки за раунд</span>
          ...
          <p className="text-xs muted">
            Очки начисляются независимо от исхода и идут в турнирный зачёт: сейчас у вас{' '}
            {formatPoints(result.gamePoints)}, место #{result.tournamentPosition}.
          </p>
        </div>
          ...
          <div className={`panel panel--pad result__reward${result.reward.collectionCompleted ? ' result__reward--complete' : ''}`}>
            ...
            {result.reward.collectionCompleted && (
              <p className="text-sm positive">
                Коллекция собрана: +{formatNumber(result.reward.bonusAwarded)} бонусных баллов и{' '}
                {formatPoints(result.reward.pointsAwarded)}. Открыта коллекция{' '}
                {result.reward.collectionLevel + 1}.
              </p>
            )}
          </div>
        )}
        ...
        <details className="panel panel--pad result__fairness">
          <summary className="text-sm">Проверить честность раунда</summary>
          <p className="text-xs muted">
            Точка краха была зафиксирована до старта. Хеш зерна вы получили в начале раунда, само зерно
            раскрывается сейчас — пересчёт доступен по ссылке.
          </p>
          <dl className="result__fairness-list text-xs">
            ...
            <div>
              <dt>Зерно клиента / nonce</dt>
              <dd className="mono">
                {result.fairness.clientSeed} / {result.fairness.nonce}
              </dd>
            </div>
            <div>
              <dt>Параметры раунда</dt>
              <dd className="mono">
                alpha={result.fairness.alpha}, edge={result.fairness.houseEdge}
              </dd>
            </div>
          </dl>
